...
- **`Sheet`** — `Dialog.Root` re-export; consumer holds the `open`/`onOpenChange` state here.
- **`BottomSheet`** — bottom-anchored chrome (portal + backdrop + slide-up panel with `data-[state]` animations). Only takes `children` and `className`.
- **`SheetTitle`** — styled `Dialog.Title`, `sr-only` by default for a11y; consumers who want a visible header render their own heading inside `BottomSheet`.
