...
export default function ExportPrivateKeyScreen() {
  ...
  if (preventionFailed) return <CaptureFailedScreen kind="privateKey" />
  ...
  const handleToggle = async () => {
    ...
    try {
      const ok = await authenticateForReveal(
        'Authenticate to export private key',
      )
  ...
  return (
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <WarningCard
        title="Warning!"
        body="Your private key grants full control over this account and its assets. Sharing or losing it will lead to permanent loss of funds."
      />

      {revealed ? (
        <ExportBox kind="privateKey" />
      ) : (
        <View style={styles.placeholderBox}>
          <Text style={styles.placeholderText}>
            Tap "Reveal" to display your private key.
          </Text>
        </View>
      )}
      ...
      <Text style={styles.footer}>
        Store this key somewhere SAFE and OFFLINE. Never share it or post it
        anywhere public. Anyone with this key can drain the account.
      </Text>
    </ScrollView>
  )
}
