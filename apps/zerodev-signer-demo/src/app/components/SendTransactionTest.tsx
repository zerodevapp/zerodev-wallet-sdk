    ...
    <div className="space-y-6">
      ...
      {mode === 'mint-nft' && !nftContractAddress && (
        <div className="flex items-start gap-2 px-4 py-3 bg-yellow-50 border border-yellow-100 rounded-lg">
          ...
          <p className="text-sm text-yellow-700">
            NFT minting is not yet available on {chain?.name}. Switch to Sepolia
            to mint NFTs.
          </p>
        </div>
      )}
      ...
      <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <p className="text-sm font-medium text-blue-900">
          Gasless transactions are active
        </p>
      </div>
      ...
      <button
        onClick={() =>
          mode === 'send-eth' ? handleSendEth() : handleMintNft()
        }
        disabled={
          loading ||
          (mode === 'send-eth' && !recipient) ||
          (mode === 'mint-nft' && !nftContractAddress)
        }
        style={{
          background:
        ...
        <div className="flex items-start gap-2 px-4 py-3 bg-red-50 border border-red-100 rounded-lg overflow-hidden">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-red-700 mt-0.5 break-words line-clamp-3">
              {error ||
                (mintError as unknown as { shortMessage?: string })
                  ?.shortMessage ||
                (sendError as unknown as { shortMessage?: string })
                  ?.shortMessage ||
                mintError?.message ||
                sendError?.message}
            </p>
      ...
      {mode === 'mint-nft' && mintTxHash && (
        <div className="space-y-2">
          ...
          <div className="px-4 py-3 bg-blue-50 border border-blue-100 rounded-lg">
            <p className="text-sm text-blue-700">NFT minted to your wallet!</p>
