...
- eccfccb: fix: extend the scoped CSS reset to SRA
...
- 2fe1800: feat: BottomSheet brand glow

  BottomSheet content now sits on a brand-colored gradient backdrop.
...
- e7f08b4: feat: add `rotate-right` icon to the shared icon set. Available via `<Icon name="rotateRight" />`; used by the SRA widget's `ErrorRetryCard` to signal a retry affordance on address-creation / route / polling failures.
- b41535d: feat: promote `Section`, `MessageDetails`, and the `camelCaseToTitle` string util from `wallet-react-ui`'s internal `signing/components/*` and `shared/utils/common.ts` to the public `@zerodev/react-ui` package barrel so other consumers (smart-routing-address-react-ui, etc.) can reuse them.
