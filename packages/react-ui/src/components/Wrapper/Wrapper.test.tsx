...
describe('Wrapper', () => {
  ...
  describe('variant backgrounds', () => {
    it('applies ghost background alpha (0.2)', () => {
      ...
      expect(outer.style.backgroundColor).toBe('rgba(255, 255, 255, 0.2)')
    })

    it('applies soft background alpha (0.5) by default', () => {
      const { container } = render(<Wrapper>Soft</Wrapper>)
      const outer = container.firstChild as HTMLElement
      expect(outer.style.backgroundColor).toBe('rgba(255, 255, 255, 0.5)')
    })

    it('applies soft background alpha (0.5) explicitly', () => {
    ...
    it('applies solid background alpha (0.8)', () => {
      ...
      const outer = container.firstChild as HTMLElement
      expect(outer.style.backgroundColor).toBe('rgba(255, 255, 255, 0.8)')
    })
  })
})
