...
describe('CodeInput', () => {
  describe('rendering', () => {
    it('renders 6 character boxes by default', () => {
      render(<CodeInput />)
      expect(screen.getAllByTestId('code-input-box')).toHaveLength(6)
    })
  ...
  describe('focus behavior', () => {
    ...
    it('marks the current character box active when focused', () => {
      const { container } = render(<CodeInput />)
      const hiddenInput = container.querySelector(
        'input[aria-label="Verification code"]',
      ) as HTMLInputElement
      ...
      const boxes = screen.getAllByTestId('code-input-box')
      // Third box (index 2) is active since we have 2 chars entered
      expect(boxes[2].hasAttribute('data-active')).toBe(true)
    })

    it('marks no box active when error is true', () => {
      render(<CodeInput error />)
      const hiddenInput = screen.getByLabelText('Verification code')

      fireEvent.focus(hiddenInput)

      for (const box of screen.getAllByTestId('code-input-box')) {
        expect(box.hasAttribute('data-active')).toBe(false)
      }
    })
  })
  ...
  describe('length prop', () => {
    it('renders the requested number of boxes (4)', () => {
      ...
      expect(screen.getAllByTestId('code-input-box')).toHaveLength(4)
    })

    it('renders the requested number of boxes (8)', () => {
      render(<CodeInput length={8} />)
      expect(screen.getAllByTestId('code-input-box')).toHaveLength(8)
    })
