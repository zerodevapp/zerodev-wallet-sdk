...
- 107915a: feat: default the AA bundler/paymaster URL to ?provider=ULTRA_RELAY

## 0.0.2

### Patch Changes

- 4590297: feat: support wagmi v3 (@wagmi/core/wagmi peers widened to ^2.22.0 || ^3.0.0) and move ox from a peer to a direct dependency so consumers no longer need to install it.
...
- Adapt to doorway-kms backend auth changes:
  ...
  - OTP code length and the magic-link URL template are now configured per-project on the backend; drop the client-supplied `otpCodeCustomization` /
    `emailCustomization`, the magic-link `redirectURL`, and wallet-react-ui's `magicLinkBaseUrl`
